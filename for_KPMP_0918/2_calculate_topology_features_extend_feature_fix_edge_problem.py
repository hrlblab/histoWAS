    ...
    *** 新增: 计算曲线的总结性特征 (AUC, Max, Min等)。 ***
    ...
    for wsi_id, group_df in combined_df.groupby('wsi_id'):
        ...
        for d in distance_list:
            default_features[f"{feature_prefix}_distance_{d}_{curve_name}"] = np.nan
        ...
        with localconverter(ro.default_converter + pandas2ri.converter + numpy2ri.converter):
            try:
                ...
                for i, d in enumerate(distance_list):
                    wsi_result[f"{feature_prefix}_distance_{d}_{curve_name}"] = gest_values_at_distances[i]
                # --- 结束原有代码 ---
    ...
    for wsi_id, group_df in combined_df.groupby('wsi_id'):
        ...
        with localconverter(ro.default_converter + pandas2ri.converter + numpy2ri.converter):
            try:
                ...
                for i, d in enumerate(distance_list):
                    wsi_result[f"{feature_prefix}_distance_{d}_{curve_name}"] = g_values_at_distances[i]
                # --- 结束原有代码 ---
...
def calculate_k_features_r_backend(area_df: pd.DataFrame,
    ...
    *** 新增: 计算L-function曲线的总结性特征 (AUC, Max, Min等)。 ***
    ...
    for wsi_id, group_df in combined_df.groupby('wsi_id'):
        ...
        for d in distance_list:
            default_features[f"{feature_prefix}_distance_{d}_{k_curve_name}"] = np.nan
            if if_L:
                default_features[f"{feature_prefix}_distance_{d}_{l_curve_name}"] = np.nan
        ...
        with localconverter(ro.default_converter + pandas2ri.converter + numpy2ri.converter):
            ...
            try:
                ...
                # --- 新增: 计算L-function总结性特征 ---
                if if_L:
                    # L(r) = sqrt(K(r)/pi), L_theo(r) = r
                    l_obs_full = np.sqrt(np.maximum(0, r_k_values) / np.pi)
                    l_theo_full = r_distances  # L_theo is just r
                ...
                for i, d in enumerate(distance_list):
                    wsi_result[f"{feature_prefix}_distance_{d}_{k_curve_name}"] = k_values_at_distances[i]

                if if_L:
                    l_values_at_distances = np.sqrt(np.maximum(0, k_values_at_distances) / np.pi) - np.array(
                        distance_list)
                    for i, d in enumerate(distance_list):
                        wsi_result[f"{feature_prefix}_distance_{d}_{l_curve_name}"] = l_values_at_distances[i]
                # --- 结束原有代码 ---
        ...
        with localconverter(ro.default_converter + pandas2ri.converter + numpy2ri.converter):
            try:
                ...
                fest_values_at_distances = np.interp(distance_list, r_distances, r_fest_values, left=np.nan,
                                                     right=np.nan)
                for i, d in enumerate(distance_list):
                    wsi_result[f"{feature_prefix}_distance_{d}_{curve_name}"] = fest_values_at_distances[i]
                # --- 结束原有代码 ---
    ...
    for wsi_id, group_df in combined_df.groupby('wsi_id'):
        ...
        with localconverter(ro.default_converter + pandas2ri.converter + numpy2ri.converter):
            try:
                ...
                for i, d in enumerate(distance_list):
                    wsi_result[f"{feature_prefix}_distance_{d}_{curve_name}"] = jest_values_at_distances[i]
...
def calculate_Kinhom_features_r_backend(area_df: pd.DataFrame,
    ...
    *** 新增: 计算Linhom-function曲线的总结性特征 (AUC, Max, Min等)。 ***
    """
    if not isinstance(process_list, list) or len(process_list) != 2:
        raise ValueError("process_list必须是一个包含两个字符串的列表。")
    ...
    is_univariate = (object_type_1 == object_type_2)
    feature_prefix = f"{object_type_1}_{object_type_2}"
    k_curve_name = "Kinhom_function"
    l_curve_name = "Linhom_function"

    # Kinhom 目前主要用于单变量分析
    if not is_univariate:
        print(f"警告: Kinhom 目前主要支持单变量分析 (e.g., ['A', 'A'])。")
        print(f"跳过 {feature_prefix} 的 Kinhom 计算。")
        return pd.DataFrame()
    ...
    for wsi_id, group_df in combined_df.groupby('wsi_id'):
        ...
        with localconverter(ro.default_converter + pandas2ri.converter + numpy2ri.converter):
            try:
                ...
                sigma <- tryCatch({ bw.ppl(ppp_obj) }, error = function(e) { (x_max_r - x_min_r) / 8 })
                ...
                r_distances = np.array(k_result_r['r'])
                r_k_values = np.array(k_result_r['iso'])
                r_k_theo = np.array(k_result_r['theo'])

                # --- 新增: 计算Linhom-function总结性特征 ---
                if if_L:
                    # L_inhom(r) = sqrt(K_inhom(r)/pi), L_theo(r) = sqrt(K_theo(r)/pi) = r
                    linhom_obs_full = np.sqrt(np.maximum(0, r_k_values) / np.pi)
                    linhom_theo_full = np.sqrt(r_k_theo / np.pi)

                    summary_feats = _calculate_curve_summary_features(
                        r_distances, linhom_obs_full, linhom_theo_full, feature_prefix, l_curve_name
                    )
                    wsi_result.update(summary_feats)
                # --- 结束新增 ---
                ...
                if if_L:
                    ...
                    l_values_at_distances = np.sqrt(np.maximum(0, k_values_at_distances) / np.pi)
                    l_theo_at_distances = np.sqrt(k_theo_at_distances / np.pi)
                    l_centered_values = l_values_at_distances - l_theo_at_distances
                    for i, d in enumerate(distance_list):
                        wsi_result[f"{feature_prefix}_distance_{d}_{l_curve_name}"] = l_centered_values[i]
                # --- 结束原有代码 ---
...
def calculate_ginhom_features_r_backend(area_df: pd.DataFrame,
    ...
    *** 新增: 计算ginhom-function曲线的总结性特征 (AUC, Max, Min等)。 ***
    """
    if not isinstance(process_list, list) or len(process_list) != 2:
        raise ValueError("process_list必须是一个包含两个字符串的列表。")
    ...
    is_univariate = (object_type_1 == object_type_2)
    feature_prefix = f"{object_type_1}_{object_type_2}"
    curve_name = "ginhom_function"

    if not is_univariate:
        print(f"警告: pcfinhom 目前主要支持单变量分析 (e.g., ['A', 'A'])。")
        print(f"跳过 {feature_prefix} 的 pcfinhom 计算。")
        return pd.DataFrame()
...
if __name__ == '__main__':
    ...
    for patient_folder in tqdm(os.listdir(input_data_path), desc="处理病患文件夹"):
        ...
        tubules_K_feature = calculate_k_features_r_backend(area_df, combined_df.copy(), ["tubules", "tubules"],
                                                           [100, 200, 300, 400])
        ...
        features_to_merge = [tubules_Kinhom_feature, tubules_ginhom_feature, ann_index_feature_df, ann_feature_df,
                             tubules_K_feature, tubules_g_feature, density_feature_df, centrography_feature_df,
                             density_stats_df, tubules_gest_feature, tubules_fest_feature, tubules_jest_feature]
        # 过滤掉空的DataFrame
