...
def get_simulation_envelope(sample_points, window_dims, func_name='Lest', n_sim=99):
    ...
    with localconverter(ro.default_converter + pandas2ri.converter + numpy2ri.converter):
        ...
        ppp_obj <- ppp(x=points_x_r, y=points_y_r, window=win)
        # 强制使用恒定强度的CSR进行模拟
        env <- envelope(ppp_obj, fun={func_name}, nsim=n_sim_r, rank=1, correction="iso", simulate=expression(runifpoint(ppp_obj$n, win=win)))
        env
        """
...
if __name__ == '__main__':
    ...
    N_SAMPLES = 1000
    DISTANCE_LIST = np.linspace(0, 400, 101)  # 用于评估的距离
    ...
    for i in tqdm(range(N_SAMPLES), desc="模拟进度"):
        ...
        if not jest_result_df.empty:
            results_curves['J'].append(jest_result_df.filter(like='_J_function').iloc[0].values)  # 获取所有J(r)
    ...
    # K-func -> L-func
    k_env = get_simulation_envelope(first_sample_points, first_sample_window, func_name='Kest', n_sim=99)
    ...
    # G-func
    g_gest_envelope = get_simulation_envelope(first_sample_points, first_sample_window, func_name='Gest', n_sim=99)
    # F-func
    f_fest_envelope = get_simulation_envelope(first_sample_points, first_sample_window, func_name='Fest', n_sim=99)
    # J-func
    j_jest_envelope = get_simulation_envelope(first_sample_points, first_sample_window, func_name='Jest', n_sim=99)
